...
191
192
193</td><td class="line-coverage quiet"><span class="cline-any cline-yes">1×</span>
...
<span class="cstat-no" title="statement not covered" >        r</span>eturn Promise.reject(new Error('Unknown domain. Make sure `address` contains a domain (ex. `bob*stellar.org`) or pass `domain` parameter when instantiating the server object.'));
      }
...
<span class="cstat-no" title="statement not covered" >          r</span>eturn Promise.reject(new BadResponseError(`Server query failed. Server responded: ${response.status} ${response.statusText}`, response.data));
        }
      });
  }
}
